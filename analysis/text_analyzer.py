    ...
    def analyze_page_content(self, page_id: int, db_session=None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if sentiment_result:
                ...
                # Update page with sentiment info using fresh query
                try:
                    fresh_page = db_session.query(Page).filter_by(id=page_id).first()
                    if fresh_page:
                        fresh_page.sentiment_score = sentiment_result.get('score', 0.0)
                        fresh_page.sentiment_label = sentiment_result.get('label', 'neutral')
                        db_session.flush()
                except Exception as e:
                    self.logger.warning(f"Could not update page sentiment for page {page_id}: {e}")
            ...
            if language_result:
                ...
                # Update page language using fresh query
                try:
                    fresh_page = db_session.query(Page).filter_by(id=page_id).first()
                    if fresh_page:
                        fresh_page.language = language_result.get('language', 'unknown')
                        db_session.flush()
                except Exception as e:
                    self.logger.warning(f"Could not update page language for page {page_id}: {e}")
