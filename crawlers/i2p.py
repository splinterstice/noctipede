...
class I2PCrawler(BaseCrawler):
    ...
    def _crawl_i2p_site(self, site: Site, db_session) -> bool:
        ...
        try:
            ...
            if content and len(content) > 1000 and not self._is_proxy_error_page(content):
                # Check for duplicate content pattern before accepting
                if self._detect_duplicate_content_pattern(content, url):
                    logger.warning(f"🚨 Direct access returned duplicate content pattern for {url}, trying true direct access")
                    # Try true direct access bypassing jump sites
                    true_direct_content = self._try_true_direct_access(url)
                    if true_direct_content and not self._detect_duplicate_content_pattern(true_direct_content, url):
                        page = self._create_page_record(url, true_direct_content, 'true_direct', db_session, site)
                        if page:
                            logger.info(f"✅ Successfully crawled {url} via true direct access - {len(true_direct_content)} chars")
                            self._deep_crawl_i2p_site(site, db_session, true_direct_content, url)
                            return True
                else:
    ...
    def _try_true_direct_access(self, url: str) -> Optional[str]:
        """Try to access URL directly through I2P proxy only, bypassing internal jump sites"""
        try:
            logger.info(f"🎯 Attempting true direct access to {url} (bypassing jump sites)")
            response = requests.get(
                url,
                proxies={'http': self.proxy_url, 'https': self.proxy_url},
                timeout=30,  # Longer timeout for direct I2P access
                headers={'User-Agent': 'Mozilla/5.0 (compatible; Noctipede/1.0)'}
            )
            ...
            if response.status_code == 200:
                ...
                if len(content) > 1000:  # Ensure substantial content
                    logger.info(f"✅ True direct access successful for {url} - {len(content)} chars")
                    return content
                else:
                    logger.info(f"⚠️ True direct access returned insufficient content for {url} ({len(content)} chars)")
                    return None
            else:
                logger.info(f"⚠️ True direct access returned HTTP {response.status_code} for {url}")
                return None
                
        except requests.exceptions.Timeout:
            logger.info(f"⏰ True direct access timeout for {url}")
            return None
        except Exception as e:
            logger.debug(f"❌ True direct access error for {url}: {e}")
            return None
    ...
    def _is_proxy_error_page(self, content: str) -> bool:
        ...
        # Check for suspicious patterns that indicate jump site error pages
        suspicious_patterns = [
    ...
    def _detect_duplicate_content_pattern(self, content: str, url: str) -> bool:
        """Detect if this content matches a pattern of identical responses from jump sites"""
        content_length = len(content)
        content_hash = hashlib.sha256(content.encode('utf-8')).hexdigest()
        
        # CRITICAL: Check for known problematic patterns first
        if content_length == 4795:
            logger.warning(f"🚨 DETECTED 4,795-character pattern for {url} - likely jump site error page!")
            # Check if this looks like a jump site error page
            content_lower = content.lower()
            jump_indicators = [
                'jump service', 'address helper', 'i2paddresshelper', 
                'you will be redirected', 'destination', 'tunnel',
                'proxy', 'service unavailable', 'not found'
            ]
            
            indicator_count = sum(1 for indicator in jump_indicators if indicator in content_lower)
            if indicator_count >= 2:
                logger.warning(f"   Contains {indicator_count} jump site indicators - REJECTING")
                return True
        
        # Store recent content patterns to detect duplicates
        if not hasattr(self, '_recent_content_patterns'):
            self._recent_content_patterns = {}
        
        # Check if we've seen this exact content length + hash combination recently
        pattern_key = f"{content_length}_{content_hash[:16]}"
        
        if pattern_key in self._recent_content_patterns:
            self._recent_content_patterns[pattern_key]['count'] += 1
            self._recent_content_patterns[pattern_key]['urls'].append(url)
            
            # If we've seen this exact content 2+ times from different sites, it's suspicious
            if self._recent_content_patterns[pattern_key]['count'] >= 2:
                logger.warning(f"🚨 Duplicate content pattern detected! Same content ({content_length} chars) from {self._recent_content_patterns[pattern_key]['count']} different sites")
                logger.warning(f"   URLs: {', '.join(self._recent_content_patterns[pattern_key]['urls'][:3])}...")
                return True
        else:
            self._recent_content_patterns[pattern_key] = {
                'count': 1,
                'urls': [url],
    ...
    def _create_page_record(self, url: str, content: str, access_method: str, db_session, site: Site) -> Optional[Page]:
        """Create and store a page record with proper URL normalization"""
        try:
            # Normalize URL to remove fragments and standardize format
            normalized_url = self._normalize_url(url)
            
            # Check if page already exists with this normalized URL
            ...
            if existing_page:
                logger.debug(f"Page already exists for normalized URL: {normalized_url}")
            ...
            storage_result = None
            try:
                storage_result = self.storage_manager.store_page_content(normalized_url, content)
                logger.info(f"📁 Stored I2P page content: {storage_result.get('object_name')}")
            except Exception as e:
                logger.warning(f"Failed to store content in MinIO: {e}")
            
            # Create page record with normalized URL AND CONTENT
            page = Page(
                site_id=site.id,
                url=normalized_url,
                title=title,  # Add title
                content=content,  # CRITICAL FIX: Store actual content
    ...
    def _extract_title(self, content: str) -> Optional[str]:
        ...
        try:
            ...
            if title_tag and title_tag.string:
                ...
                # Truncate title if too long (database constraint)
                if len(title) > 1900:
                    title = title[:1900] + "..."
                    logger.warning(f"Title truncated to 1900 characters")
                return title
        except Exception as e:
    ...
    def _try_true_direct_access(self, url: str) -> Optional[str]:
        """Try true direct access using curl to bypass jump sites completely"""
        try:
            import subprocess
            logger.info(f"🔧 Attempting TRUE DIRECT ACCESS to {url} using curl...")
            
            # Use curl with I2P proxy for direct access
            cmd = [
                'curl', '-s', '--connect-timeout', '15', '--max-time', '30',
                '-x', 'http://i2p-proxy:4444',
                '--user-agent', 'Mozilla/5.0 (compatible; Noctipede-Direct/1.0)',
                url
            ]
            ...
            if result.returncode == 0 and result.stdout:
                ...
                if len(content) > 2000:
                    ...
                    # Verify this looks like real content, not an error page
                    content_lower = content.lower()
                    if any(indicator in content_lower for indicator in ['<html', '<!doctype', '<title>']):
                        return content
                    else:
                        logger.warning(f"⚠️ TRUE DIRECT ACCESS returned non-HTML content for {url}")
                        return None
                else:
                    logger.warning(f"⚠️ TRUE DIRECT ACCESS returned insufficient content for {url}: {len(content)} chars")
                    return None
            else:
                logger.warning(f"⚠️ TRUE DIRECT ACCESS failed for {url}: {result.stderr}")
                return None
                
        except Exception as e:
            logger.error(f"Error in true direct access for {url}: {e}")
            return None
    ...
    def _deep_crawl_i2p_site(self, site: Site, db_session, initial_content: str, base_url: str) -> None:
        ...
        try:
            ...
            crawled_pages = {self._normalize_url(base_url)}  # Already crawled the homepage (normalized)
            ...
            while pages_to_crawl and len(crawled_pages) < 20:  # Limit to 20 pages per I2P site
                ...
                if normalized_current_url in crawled_pages:
                    continue
                ...
                if content and len(content) > 500 and not self._is_proxy_error_page(content):
                    # Create page record (this will normalize the URL internally)
                    page = self._create_page_record(current_url, content, f'deep_crawl_d{current_depth}', db_session, site)
                    if page:
                        crawled_pages.add(normalized_current_url)  # Add normalized URL to prevent duplicates
                        logger.info(f"✅ Deep crawled: {normalized_current_url} - {len(content)} chars")
                        ...
                        if current_depth < self.settings.max_crawl_depth:
                            try:
                                ...
                                for link in page_links[:10]:
                                    ...
                                    if normalized_link not in crawled_pages and not any(self._normalize_url(url) == normalized_link for url, _, _ in pages_to_crawl):
