...
from database.session_fix import ensure_session_bound, get_fresh_object
from database.models import Site, Page
...
class BaseCrawler(ABC):
    ...
    def _crawl_site_pages(self, site: Site, db_session) -> bool:
        ...
        # Store URL and ID for error handling
        site_url = site.url if site else "unknown"
        site_id = site.id if site else None
        
        try:
            # Get fresh site object from database to avoid session issues
            site = get_fresh_object(Site, site_id, db_session)
            if not site:
                self.logger.error(f"Site with ID {site_id} not found in database")
                return False
            
            # Update site_url after getting fresh site object
            site_url = site.url
    ...
    def _crawl_page(self, url: str, site_id: int, db_session) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Truncate title if too long (defensive programming)
            if title and len(title) > 1900:  # Leave buffer below 2000 limit
                original_length = len(title)
                title = title[:1900] + "..."
                self.logger.warning(f"Truncated long title for {url} (original: {original_length} chars)")
            ...
            if existing_page:
                # Check if existing page was truncated at old 10KB limit
                settings = get_settings()
                max_content_size = settings.max_content_size
                
                # If existing page is exactly 10KB and new limit is higher, re-crawl it
                if (len(existing_page.content or '') == 10000 and max_content_size > 10000):
                    self.logger.info(f"Re-crawling truncated page (was 10KB): {normalized_url}")
                    # Update existing page with new content
                    existing_page.content = content[:max_content_size]
                    existing_page.content_hash = content_hash
                    existing_page.content_length = len(content)
                    existing_page.crawled_at = datetime.utcnow()
                    existing_page.response_time = response_time
                    db_session.flush()
            ...
            # Get configurable content size limit
            settings = get_settings()
            max_content_size = settings.max_content_size
            ...
            page = Page(
                ...
                title=title,
                content=content[:max_content_size],  # Store content up to configured limit
            ...
            # Update site page count - Get fresh site object from database
            try:
                site = get_fresh_object(Site, site_id, db_session)
                if site:
                    site.page_count = (site.page_count or 0) + 1
                    # Flush immediately to catch any update errors
                    db_session.flush()
                else:
                    self.logger.warning(f"Site with ID {site_id} not found for page count update")
            except Exception as e:
                self.logger.warning(f"Could not update site page count for site {site_id}: {e}")
                # Continue without failing the entire operation
            
            # Flush to ensure page.id is available for AI analysis (separate from site update)
            try:
                db_session.flush()
            except Exception as e:
                self.logger.error(f"Could not flush page data: {e}")
            ...
            if self.analysis_manager and self.settings.content_analysis_enabled:
                try:
                    ...
                    # Use page ID for AI analysis to avoid session issues
                    if page and page.id:
                        ...
                        self.logger.error(f"No page ID available for AI analysis: {url}")
                except Exception as e:
                    self.logger.error(f"AI analysis error for page {normalized_url}: {e}")
            
            # Extract and store media files using normalized URL
            # Get fresh page object for media extraction
            if page and page.id:
                fresh_page = get_fresh_object(Page, page.id, db_session)
                if fresh_page:
                    media_files = self._extract_media_files(soup, normalized_url, fresh_page.id, db_session)
                else:
                    self.logger.warning(f"Could not get fresh page object for media extraction: {url}")
                    media_files = []
            else:
                self.logger.warning(f"No page ID available for media extraction: {url}")
                media_files = []
            ...
            # Get fresh page object before returning to avoid session issues
            if page and page.id:
                fresh_page = get_fresh_object(Page, page.id, db_session)
                page = fresh_page if fresh_page else page
    ...
    def _download_and_store_media(self, url: str, page_id: int, db_session) -> Optional[MediaFile]:
        ...
        try:
            ...
            storage_result = self.storage_manager.store_media_file(url, content, content_type)
            ...
            media_file = MediaFile(
                ...
                minio_object_name=storage_result.get('object_name'),
                downloaded_at=datetime.utcnow()
            )
