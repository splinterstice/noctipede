...
class EnhancedMetricsCollector:
    ...
    async def collect_all_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tasks = [
                ...
                self.collect_crawler_status_codes()
            ]
            
            results = await asyncio.gather(*tasks, return_exceptions=True)
            
            # Assign results
            metric_keys = ['system', 'database', 'minio', 'ollama', 'crawler', 'network', 'services', 'http_status_codes']
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.error(f"Error collecting {metric_keys[i]} metrics: {result}")
                    if i < len(metric_keys) - 1:  # Regular metrics
                        metrics[metric_keys[i]] = {'error': str(result), 'status': 'error'}
                    else:  # HTTP status codes
                        metrics['http_status_codes'] = {'error': str(result), 'status': 'error'}
                else:
                    if i < len(metric_keys) - 1:  # Regular metrics
                        metrics[metric_keys[i]] = result
                    else:  # HTTP status codes - merge with database metrics
                        metrics['http_status_codes'] = result
    ...
    async def collect_crawler_status_codes(self) -> Dict[str, Any]:
        """Collect HTTP status codes from recent crawler logs"""
        try:
            import subprocess
            import re
            
            # Get recent crawler logs from Kubernetes
            try:
                # Try to get logs from running crawler jobs
                result = subprocess.run([
                    'kubectl', 'get', 'jobs', '-n', 'noctipede', '-o', 'name'
                ], capture_output=True, text=True, timeout=5)
                
                if result.returncode == 0:
                    jobs = result.stdout.strip().split('\n')
                    crawler_jobs = [job for job in jobs if 'crawler' in job.lower()]
                    
                    status_codes = {}
                    
                    for job in crawler_jobs[-3:]:  # Check last 3 crawler jobs
                        try:
                            log_result = subprocess.run([
                                'kubectl', 'logs', job, '-n', 'noctipede', '--tail=1000'
                            ], capture_output=True, text=True, timeout=10)
                            
                            if log_result.returncode == 0:
                                # Parse status codes from logs
                                # Pattern: "Non-200 status for ... : 404"
                                pattern = r'Non-200 status for .+?: (\d{3})'
                                ...
                                # Also look for HTTP status patterns
                                # Pattern: "HTTP 404" or "status 404"
                                http_pattern = r'(?:HTTP|status)\s+(\d{3})'
                                http_matches = re.findall(http_pattern, log_result.stdout, re.IGNORECASE)
                                
                                for status_code in http_matches:
                                    status_code = int(status_code)
                                    if status_code != 200:  # Don't double-count 200s
                                        status_codes[status_code] = status_codes.get(status_code, 0) + 1
                        
                        except Exception as e:
                            self.logger.debug(f"Could not get logs from {job}: {e}")
                            continue
                    
                    return {
                        'status': 'success',
                        'recent_status_codes': status_codes,
                        'total_non_200': sum(status_codes.values()),
                        'unique_error_codes': len(status_codes)
                    }
                
            except Exception as e:
                self.logger.debug(f"Could not collect crawler status codes: {e}")
                
            return {
                'status': 'unavailable',
                'error': 'Could not access crawler logs',
                'recent_status_codes': {},
                'total_non_200': 0,
            ...
            self.logger.error(f"Error collecting crawler status codes: {e}")
            return {
                'status': 'error',
                'error': str(e),
                'recent_status_codes': {},
        ...
        try:
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    ORDER BY count DESC
                """)
                site_statuses = dict(cursor.fetchall())
                
                # Network-specific status codes
                cursor.execute("""
                    SELECT 
                        CASE 
                        ...
                        COUNT(*) as count
                    FROM pages 
                    GROUP BY network, status_code
                    ORDER BY network, count DESC
                """)
                network_status_codes = {}
                for network, status_code, count in cursor.fetchall():
                    if network not in network_status_codes:
                        network_status_codes[network] = {}
                    network_status_codes[network][status_code] = count
                ...
                metrics['noctipede'] = {
                    ...
                    'network_status_codes': network_status_codes
                }
            except Exception as e:
