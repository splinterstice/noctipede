...
class CombinedMetricsCollector:
    ...
    async def collect_all_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Assign results
            metric_keys = ['system', 'database', 'minio', 'ollama', 'crawler', 'network', 'services', 'crawler_status_codes']
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    else:  # Crawler status codes
                        metrics['crawler_status_codes'] = {'error': str(result), 'status': 'error'}
                else:
                    if i < len(metric_keys) - 1:  # Regular metrics
                        metrics[metric_keys[i]] = result
                    else:  # Crawler status codes
                        metrics['crawler_status_codes'] = result
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    for job in crawler_jobs[-2:]:  # Check last 2 crawler jobs
                        try:
                            log_result = subprocess.run([
                                'kubectl', 'logs', job, '-n', 'noctipede', '--tail=500'
                    ...
                    return {
                        'status': 'success',
                        'recent_error_codes': status_codes,
