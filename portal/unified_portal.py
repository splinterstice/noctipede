...
<body>
    <div class="container">
        ...
        @self.app.get("/ai-reports", response_class=HTMLResponse)
        async def ai_reports(request: Request):
            """AI Reports dashboard for natural language querying"""
            return self.templates.TemplateResponse(
                "ai_reports.html", 
                {"request": request}
            )
        ...
        async def get_crawler_metrics():
            ...
            try:
                crawler_metrics = await self.metrics_collector.collect_all_metrics()
                # Extract just the crawler portion
                return JSONResponse(crawler_metrics.get("crawler", {}))
            except Exception as e:
        ...
        async def get_system_metrics():
            ...
            try:
                ...
                return JSONResponse(all_metrics.get("system", {}))
            except Exception as e:
                logger.error(f"Error getting system metrics: {e}")
                return JSONResponse({"error": str(e)}, status_code=500)
        
        @self.app.get("/api/enhanced-metrics")
        async def get_enhanced_metrics():
            """Get enhanced metrics for the enhanced dashboard"""
            try:
                # Try to get enhanced metrics from the combined collector
                enhanced_metrics = await self.metrics_collector.collect_all_metrics()
                return JSONResponse(enhanced_metrics)
            except Exception as e:
                logger.error(f"Error getting enhanced metrics: {e}")
                # Fallback to basic metrics with enhanced structure
                try:
                    all_metrics = await self.metrics_collector.collect_all_metrics()
                    basic_metrics = all_metrics.get("crawler", {})
                    fallback_data = {
                        "crawler": basic_metrics,
                        "system": {"status": "basic_fallback"},
                        "database": {"status": "unknown"},
                        "minio": {"status": "unknown"},
                        "ollama": {"status": "unknown"},
                        "network": {"status": "unknown"},
                        "services": {"status": "basic_mode"},
                        "timestamp": datetime.now().isoformat(),
                        "collection_time": "0.1",
                        "status": "basic_fallback",
                        "message": "Enhanced metrics require additional dependencies"
                    }
                    return JSONResponse(fallback_data)
                except Exception as e2:
                    logger.error(f"Error getting fallback metrics: {e2}")
                    return JSONResponse({"error": str(e), "fallback_error": str(e2)}, status_code=500)
        ...
        async def get_readiness():
            ...
            try:
                ...
                all_metrics = await self.metrics_collector.collect_all_metrics()
                network_metrics = all_metrics.get("network", {})
